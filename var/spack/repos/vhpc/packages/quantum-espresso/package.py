...
class QuantumEspresso(Package):
    ...
    version('6.3MaX', sha256='9e130a8042b244964f4b7b04ef3a694f610610d166ae809c4a045aa603633688')
    ...
    variant('cuda_arch', default='35', description='GPU architecture',
            values=('35', '60', '70'), multi=False)

    # Dependencies
    depends_on('blas')
    depends_on('lapack')
    depends_on('blas threads=openmp', when='+openmp')
    depends_on('blas threads=openmp', when='+openmp +cuda')
    depends_on('fftw-api@3')
#    depends_on('intel-mkl', when='+cuda')
    # PGI asplodes when compiling any 'generic-*' simd,
    # and we can't be sure that the machine has any kind of AVX,
    # so we play safe and settle for the next best option
    depends_on('fftw-api@3 +openmp', when='+openmp')
    depends_on('fftw-api@3 +openmp', when='+cuda')
    depends_on('fftw-api@3 simd=sse2', when='%pgi')
    depends_on('fftw +openmp simd=sse2', when=' %pgi +cuda +openmp')
    ...
    depends_on('pgi@17.10: +mpi +nvidia', when='+cuda')
...
#   compiler_warning = 'Unly PGI 17.10+ is supported when building with +cuda'
#   unsupported_cuda_compilers = [
#       '%arm', '%cce', '%clang',
#       '%fj', '%gnu', '%intel',
#       '%nag', '%xl', '%xl_r'
#   ]

#   for compiler in unsupported_cuda_compilers:
#       conflicts(
#           compiler,
#           when='+cuda {0}'.format(compiler),
#           msg=compiler_warning)

    conflicts('~openmp', when='+cuda',
              msg='OpenMP is required when building +cuda')
    ...
    @property
    def pgi_cuda_version(self):
        output = sp.check_output(['pgcc', '-show'], stderr=sp.PIPE)
        value = str(next((line for line in output.decode().splitlines()
                    if 'DEFCUDAVERSION' in line), '')).split('=')[1]
        return '{0}'.format(Version(value))

    @property
    def pgi_cuda_prefix(self):
        output = sp.check_output(['pgcc', '-show'], stderr=sp.PIPE)
        value = str(next((line for line in output.decode().splitlines()
                    if 'USECUDAROOT' in line), '')).split('=')[1]
        return '/'.join([value, self.pgi_cuda_version])

    @property
    def pgi_mpi_bindir(self):
        spec = self.spec
        mpi = find(spec['pgi'].prefix, '/bin/mpirun', recursive=True)
        return os.path.dirname(str(mpi[0]))
    ...
    def install(self, spec, prefix):
        ...
        if '+mpi' in spec:
            ...
            mpi = self.pgi_mpi_bindir
            options.append('--enable-parallel=yes')
            options.append('MPIF90={0}'.format(join_path(mpi, 'mpifc')))
            options.append('CC={0}'.format(join_path(mpi, 'mpicc')))
        else:
        ...
        if '^fftw@3: +openmp' in spec:
            fftw_prefix = spec['fftw'].prefix
            options.append('FFTW_INCLUDE={0}'.format(fftw_prefix.include))
            fftw_libs = LibraryList([
                fftw_prefix.lib.join('libfftw3_omp.so'),
                fftw_prefix.lib.join('libfftw3.so')
            ])
            fftw_ld_flags = fftw_libs.ld_flags
            options.append('FFT_LIBS={0}'.format(fftw_ld_flags))
            tty.debug(fftw_ld_flags)
        ...
        if '+cuda' in spec:
            cuda_prefix = self.pgi_cuda_prefix
            cuda_version = self.pgi_cuda_version
            cuda_arch = spec.variants['cuda_arch'].value
            options.extend([
                '--with-cuda={0}'.format(cuda_prefix),
                '--with-cuda-runtime={0}'.format(cuda_version),
                '--with-cuda-cc={0}'.format(cuda_arch),
            ])
